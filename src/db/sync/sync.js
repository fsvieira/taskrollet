...
export default function sync(context, url, { token }, baseRevision, syncedRevision, changes, partial, applyRemoteChanges, onChangesAccepted, onSuccess, onError) {
    ...
    ws.onmessage = function (event) {
        try {
            ...
            } else if (requestFromServer.type == "syncDisabled") {
                // TODO: we need to send this error to interface.
                var requestId = requestFromServer.requestId;
                ws.close();
                onError(requestFromServer.message, Infinity); // Don't reconnect connection will be rejeced by server.
            }
